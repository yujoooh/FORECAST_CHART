...
HOME_PATH = "C:\\Users\\HGChoe\\Desktop\\geosr_map\\ImageDownUpload"
DOWNLOAD_PATH = "C:\\Users\\HGChoe\\Downloads"
OBJECT_PATH = "C:\\Users\\HGChoe\\Desktop\\geosr_map\\ImageDownUpload\\downloads"
CONID = "OCEAN_WEB"
CONPW = "infoshfl03"
CONINFO = "10.27.90.16:1521/mgis"

day0 =  datetime.datetime.now().strftime('%Y%m%d')
day1 = (datetime.datetime.now() + datetime.timedelta(1)).strftime('%Y%m%d')
day2 = (datetime.datetime.now() + datetime.timedelta(2)).strftime('%Y%m%d')
...
def down_upload_loop(liType, liDate):
    ...
    # URL Information
    # baseURL = 'http://localhost:8080/daily_ocean2020/'
    baseURL = 'http://geosr.server/daily_ocean/'
            ...
            for hour in liDate[date]:
                for try_index in range(5): ### If some error occurs, Loop 5 times.
                    try: ### MAIN
                        ...
                        #     sqlStr += f"set "
                        #     sqlStr += f"FILE_NAME  = '{regType+str(date)+str(hour).zfill(2)+'.png'}', "
                        ...
                        #     sqlStr += f"values ('{regType[3:].upper()}', '{ddic}', '{str(hour).zfill(2)}', '{regType+str(date)+str(hour).zfill(2)+'.png'}', NULL, 'Y', :image) "
